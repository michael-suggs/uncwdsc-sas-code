...
/***You didn't really need to do this for the exercise, but it is some good work.
   If you'd like a potentially easier way, look up the use of PIPE in the FILENAME statement***/
...
/***What assumption are you making (beyond the set of variables in the text files) that
     may not hold up if more data is introduced (it isn't a bad assumption)?
    Is FIRSTOBS= always 2?
    
    It absolutely doesn't have to be--I'm always assuming the presence of
    a header row, which definitely isn't always present (nor always just one line).
    Instead, this could be made variable and passed in as a macro parameter.
 ***/
